...
本项目通过 AI 辅助开发创建并维护。代码、构建脚本、文档和 CI/CD 配置在用于生产环境前应仔细审查和测试。
...
1. 从[最新版本](https://github.com/9bow/OST/releases/latest)下载 `OST.zip`
...
首次启动时，macOS 可能会提示授予以下权限。如未提示，请手动启用：

| 权限 | 用途 | 启用方法 |
|---|---|---|
| **屏幕录制** | 通过 ScreenCaptureKit 捕获系统音频 | 系统设置 > 隐私与安全性 > 屏幕与系统音频录制 > 启用 OST |
| **系统音频录制** | macOS 15+ 的系统音频捕获权限 | 系统设置 > 隐私与安全性 > 屏幕与系统音频录制 > 启用 OST |
| **语音识别** | SFSpeechRecognizer 访问权限 | 系统设置 > 隐私与安全性 > 语音识别 > 启用 OST |

> 如果是在系统设置中手动启用权限，请重新启动 OST 以使更改生效。
...
3. 下载完成后，在 OST 设置 > 语言标签页中确认 **"设备端识别"** 仍保持开启
...
`./test.sh` 仅使用系统命令行工具，并运行文档、工作流、回归、行为和类型检查关卡。
需要真实 macOS 权限、音频捕获、Apple Translation 语言包或在线备用翻译网络行为的发布检查，请使用 [docs/manual-qa.md](docs/manual-qa.md)。
...
- **合并**：在单个窗口中显示原文和译文
- **分离**：默认模式，将识别（原文）和翻译分为两个独立窗口。每个窗口可独立放置和调整大小。菜单栏锁定/解锁会同时应用于两个窗口；设置中可单独锁定每个窗口

### 提示

- **语音停顿**：在 设置 > 显示 > "Speech Pause" 滑块中调整（默认 3 秒）。较短的值更快确定文本；较长的值等待自然句子结束
- **字幕过期**：旧字幕在设定时间后自动消失（默认 20 秒）
- **最大行数**：控制同时显示的字幕条目数（默认 3）
- **会话历史**：默认开启。通过菜单栏 > Session History 查看并导出过去的语音识别会话，也可在 设置 > 调试 中关闭保存
- **设备端识别**：默认开启。如果所选语言模型不可用，或你想使用服务器端识别，请在 设置 > 语言 中关闭
- **在线备用翻译**：默认关闭。仅当你希望 OST 在 Apple Translation 不可用时将文本发送到 Google Translate，才在 设置 > 语言 中启用
...
| 问题 | 解决方法 |
|---|---|
| 未捕获到音频 | 授予屏幕录制和系统音频录制权限。如果是在系统设置中更改的权限，请重新启动 OST |
| 语音识别不工作 | 授予语音识别权限；确保已启用 Siri 和听写 |
| 翻译未显示 | 下载翻译语言包，或在允许将文本发送到 Google Translate 时于 设置 > 语言 中启用在线备用翻译 |
| 叠加层不可见但阻止点击 | 使用 设置 > 显示 > "Reset All Overlay Windows" 恢复默认位置 |
| macOS 阻止应用运行 | 已安装应用运行 `xattr -dr com.apple.quarantine /Applications/OST.app`，本地构建运行 `xattr -dr com.apple.quarantine build/OST.app` |

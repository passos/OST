...
이 프로젝트는 AI 지원 개발을 통해 생성 및 유지관리되었습니다. 코드, 빌드 스크립트, 문서, CI/CD 구성은 프로덕션 사용 전에 신중하게 검토하고 테스트해야 합니다.
...
1. [최신 릴리즈](https://github.com/9bow/OST/releases/latest)에서 `OST.zip` 다운로드
...
처음 실행 시 macOS가 다음 권한을 요청할 수 있습니다. 요청이 나타나지 않으면 수동으로 활성화하세요:

| 권한 | 용도 | 활성화 방법 |
|---|---|---|
| **화면 기록** | ScreenCaptureKit을 통한 시스템 오디오 캡처 | 시스템 설정 > 개인정보 보호 및 보안 > 화면 및 시스템 오디오 녹음 > OST 활성화 |
| **시스템 오디오 녹음** | macOS 15+의 시스템 오디오 캡처 권한 | 시스템 설정 > 개인정보 보호 및 보안 > 화면 및 시스템 오디오 녹음 > OST 활성화 |
| **음성 인식** | SFSpeechRecognizer 접근 | 시스템 설정 > 개인정보 보호 및 보안 > 음성 인식 > OST 활성화 |

> 시스템 설정에서 권한을 수동으로 활성화했다면 변경 사항 적용을 위해 OST를 재시작하세요.
...
3. 다운로드 후 OST 설정 > 언어 탭에서 **"온디바이스 인식"** 이 켜져 있는지 확인
...
`./test.sh`는 시스템 command-line tools만 사용하며 문서, 워크플로, 회귀, 동작, 타입 체크 게이트를 실행합니다.
실제 macOS 권한, 오디오 캡처, Apple Translation 언어 팩 또는 온라인 대체 번역 네트워크 동작이 필요한 릴리즈 검사는 [docs/manual-qa.md](docs/manual-qa.md)를 사용하세요.
...
3. **Start Capture**를 클릭하여 시스템 오디오 캡처 시작
...
- **분할**: 기본 모드이며 인식 (원문)과 번역을 두 개의 별도 창으로 분리. 각 창을 독립적으로 배치 및 크기 조절 가능. 메뉴 바 잠금/잠금 해제는 두 창에 동시에 적용되며, 설정에서는 각 창을 독립적으로 잠글 수 있음

### 팁

- **음성 중단**: 설정 > 디스플레이 > "Speech Pause" 슬라이더로 조절 (기본값 3초). 짧은 값은 텍스트를 더 빠르게 확정하고, 긴 값은 자연스러운 문장 끝을 기다림
- **자막 만료**: 오래된 자막은 설정된 시간 후 자동으로 사라짐 (기본값 20초)
- **최대 줄 수**: 동시에 표시되는 자막 항목 수를 조절 (기본값 3)
- **세션 기록**: 기본적으로 활성화되어 있습니다. 메뉴 바 > Session History에서 과거 음성 인식 세션을 확인하고 내보내거나, 설정 > 디버그에서 저장을 비활성화할 수 있습니다
- **온디바이스 인식**: 기본적으로 활성화되어 있습니다. 선택한 언어 모델이 없거나 서버 기반 인식을 사용하려면 설정 > 언어에서 비활성화하세요
- **온라인 대체 번역**: 기본적으로 비활성화되어 있습니다. Apple Translation을 사용할 수 없을 때 OST가 텍스트를 Google Translate로 보내도 되는 경우에만 설정 > 언어에서 활성화하세요
...
| 문제 | 해결 방법 |
|---|---|
| 오디오가 캡처되지 않음 | 화면 기록 및 시스템 오디오 녹음 권한을 허용하세요. 시스템 설정에서 변경했다면 OST를 재시작하세요 |
| 음성 인식이 작동하지 않음 | 음성 인식 권한 부여; Siri 및 받아쓰기 활성화 확인 |
| 번역이 나타나지 않음 | 번역 언어 팩을 다운로드하거나, 텍스트를 Google Translate로 보내도 괜찮다면 설정 > 언어에서 온라인 대체 번역 활성화 |
| 오버레이가 보이지 않지만 클릭을 차단함 | 설정 > 디스플레이 > "Reset All Overlay Windows"로 기본 위치 복원 |
| macOS가 앱을 차단함 | 설치한 앱은 `xattr -dr com.apple.quarantine /Applications/OST.app`, 로컬 빌드는 `xattr -dr com.apple.quarantine build/OST.app` 실행 |
